...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=projetofinal.example.com
...
workon projetofinal  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development projetofinal npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop projetofinal
    docker rm projetofinal
    docker run --name projetofinal -d --env-file /home/ubuntu/projetofinal.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/projetofinal:/dkdata \
        projetofinal start_web.sh
}
...
echo_green "Welcome to projetofinal's dev env"
